...
int main(){
    ...
    /*for(int j = 0; j < strlen(str)/2 + strlen(str)%2; j++){
        if(m[j][k] == 1){
            for(int i = 0; i < k; i++){
                if(m[j][i] == 1)
                    matrix_adj[i][i]+=1;
    ...
    int new_inc[r][strlen(str)/2 + strlen(str)%2];
    int new = 0;
    for(j = 0; j < strlen(str)/2 + strlen(str)%2; j++){
        if( m[j][k] == 2){
            if(new == 0){
                for(i = 0; i < r; i++){
                    new_inc[i][new]=matrix_inc[i][j];
                }
                new+=1;
            }
            else{
